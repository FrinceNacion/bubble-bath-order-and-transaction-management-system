...
const ReportsPage = () => {
  ...
  const fetchUser = async () => {
    try {
      const res = await fetch(API_ENDPOINTS.AUTH.GET_USER, {
        method: 'POST',
        credentials: 'include'
      });
  ...
  const exportPDF = () => {
    ...
    try {
      ...
      // Branding Header
      doc.addImage(bubblebathLogo, "PNG", 14, 10, 18, 16);
      doc.setFontSize(22);
      doc.setFont("helvetica", "bold");
      doc.setTextColor(52, 152, 219); // Primary Blue
      doc.text("BUBBLE BATH", 38, 18);

      doc.setFontSize(10);
      doc.setFont("helvetica", "normal");
      doc.setTextColor(100, 100, 100);
      doc.text("Laundry & Dry Cleaning Service", 38, 23);

      // Report Info
      doc.setFontSize(14);
      doc.setFont("helvetica", "bold");
      doc.setTextColor(44, 62, 80);
      doc.text(`${reportType.toUpperCase()} ANALYTICAL REPORT`, 14, 45);

      doc.setFontSize(9);
      doc.setFont("helvetica", "normal");
      doc.text(`Reporting Period: ${startDate} to ${endDate}`, 14, 52);
      doc.text(`Generated By: ${user.name} - ${user.role}`, 14, 57);
      ...
      doc.save(`BB_Report_${reportType}_${startDate}_${endDate}.pdf`);
  ...
  return (
    <main className="container flex-fill p-4 p-xl-5">
      ...
      <div className="row g-3 mb-4">
        ...
        <div className="col-md-3">
          <div className="card border-0 shadow-sm h-100 p-3 d-flex justify-content-evenly">
            <div className="d-flex justify-content-between align-items-center">
              <div>
                <p className="text-secondary small mb-1">Growth (Month-over-Month)</p>
